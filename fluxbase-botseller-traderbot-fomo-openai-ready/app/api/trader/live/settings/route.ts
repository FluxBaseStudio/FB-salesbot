...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ settings: overview.settings, liveTradingEnvEnabled: overview.liveTradingEnvEnabled }, { headers: traderNoStoreHeaders });
  } catch (error) {
    return traderJsonError(traderErrorMessage(error, "Blad odczytu ustawien live tradingu."), 500);
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return traderJsonError(traderErrorMessage(error, "Blad zapisu ustawien live tradingu."), 500);
  }
}
