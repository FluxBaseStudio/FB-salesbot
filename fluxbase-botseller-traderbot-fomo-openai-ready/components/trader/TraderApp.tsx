...
import FomoCopyTradingView from "@/components/trader/FomoCopyTradingView";
...
import TraderAiBotsView, { type TraderAiBotDraft } from "@/components/trader/TraderAiBotsView";
...
import type { Candle, TraderAiBotsOverview, TraderCopyOverview, TraderOverview, TraderSettings, TraderTab } from "@/lib/trader/types";
...
const emptyCopyOverview: TraderCopyOverview = {
  settings: {
    copy_enabled: false,
    copy_position_amount: TRADER_DEFAULT_SETTINGS.copy_position_amount,
    copy_min_source_score: TRADER_DEFAULT_SETTINGS.copy_min_source_score,
    copy_min_confidence_score: TRADER_DEFAULT_SETTINGS.copy_min_confidence_score,
    copy_max_signal_age_seconds: TRADER_DEFAULT_SETTINGS.copy_max_signal_age_seconds,
    copy_max_price_drift_percent: TRADER_DEFAULT_SETTINGS.copy_max_price_drift_percent,
    copy_require_ai_review: TRADER_DEFAULT_SETTINGS.copy_require_ai_review,
    copy_min_ai_confidence_score: TRADER_DEFAULT_SETTINGS.copy_min_ai_confidence_score,
    copy_promote_to_live_proposals: TRADER_DEFAULT_SETTINGS.copy_promote_to_live_proposals,
    paper_enabled: false,
    paper_balance: TRADER_DEFAULT_SETTINGS.paper_balance,
...
const emptyAiBotsOverview: TraderAiBotsOverview = {
  bots: [],
  runs: [],
  globalApiKeyAvailable: false,
  metrics: {
    activeBots: 0,
    marketBots: 0,
    fomoBots: 0,
...
export default function TraderApp() {
  ...
  const [copyOverview, setCopyOverview] = useState<TraderCopyOverview>(emptyCopyOverview);
  const [aiBotsOverview, setAiBotsOverview] = useState<TraderAiBotsOverview>(emptyAiBotsOverview);
  ...
  async function load(explicitToken?: string) {
    ...
    try {
      ...
      void Promise.all([loadCopy(explicitToken), loadAiBots(explicitToken)]);
    } catch (error) {
  ...
  async function loadCopy(explicitToken?: string) {
    try {
      const data = await api<TraderCopyOverview>("/api/trader/copy/overview", {}, explicitToken);
      setCopyOverview(data);
    } catch (error) {
      setNotice({ tone: "warning", message: `FOMO Copy: ${errorMessage(error)}` });
    }
  }

  async function loadAiBots(explicitToken?: string) {
    try {
      const data = await api<TraderAiBotsOverview>("/api/trader/ai-bots/overview", {}, explicitToken);
      setAiBotsOverview(data);
    } catch (error) {
      setNotice({ tone: "warning", message: `Boty OpenAI: ${errorMessage(error)}` });
    }
  }
  ...
  async function runCopyAction(action: () => Promise<void>, success: string) {
    setBusy(true);
    try {
      await action();
      await Promise.all([load(), loadCopy()]);
      setNotice({ tone: "success", message: success });
    } catch (error) {
      setNotice({ tone: "danger", message: errorMessage(error) });
    } finally {
  ...
  async function runAiBotsAction(action: () => Promise<void>, success: string) {
    setBusy(true);
    try {
      await action();
      await Promise.all([load(), loadAiBots(), loadCopy()]);
      setNotice({ tone: "success", message: success });
    } catch (error) {
      setNotice({ tone: "danger", message: errorMessage(error) });
    } finally {
    ...
    <DashboardShell
      ...
      sidebar={
        <TraderSidebar
          ...
          onResetEmergencyStop={() => void runAction(
            () => api("/api/trader/emergency-stop/reset", { method: "POST" }),
            "Awaryjna blokada została zresetowana.",
          )}
        />
      }
    >
      ...
      {tab === "copy" ? (
        <FomoCopyTradingView
          ...
          busy={busy}
          onRefresh={() => void loadCopy()}
          onSaveSettings={(patch) => void runCopyAction(
            () => api("/api/trader/copy/settings", { method: "POST", body: JSON.stringify(patch) }),
            "Ustawienia FOMO Copy zostały zapisane.",
          )}
          onAddSource={(payload) => void runCopyAction(
            () => api("/api/trader/copy/sources", { method: "POST", body: JSON.stringify(payload) }),
            "Źródło FOMO zostało dodane.",
          )}
          onToggleSource={(id, isActive) => void runCopyAction(
            () => api(`/api/trader/copy/sources/${id}/toggle`, { method: "POST", body: JSON.stringify({ is_active: isActive }) }),
            isActive ? "Źródło FOMO zostało włączone." : "Źródło FOMO zostało wyłączone.",
          )}
          onImportSignal={(payload) => void runCopyAction(
            () => api("/api/trader/copy/signals", { method: "POST", body: JSON.stringify(payload) }),
            "Sygnał FOMO trafił do kolejki analizy.",
          )}
          onRun={() => void runCopyAction(
            () => api("/api/trader/copy/run", { method: "POST" }),
            "Silnik FOMO Copy przeanalizował kolejny sygnał.",
          )}
        />
      ) : null}
      ...
      {tab === "ai_bots" ? (
        <TraderAiBotsView
          overview={aiBotsOverview}
          busy={busy}
          onRefresh={() => void loadAiBots()}
          onSave={(draft: TraderAiBotDraft) => void runAiBotsAction(
            () => api("/api/trader/ai-bots", { method: "POST", body: JSON.stringify(draft) }),
            draft.id ? "Bot OpenAI został zaktualizowany." : "Bot OpenAI został dodany.",
          )}
          onToggle={(id, active) => void runAiBotsAction(
            () => api(`/api/trader/ai-bots/${id}/toggle`, { method: "POST", body: JSON.stringify({ active }) }),
            active ? "Bot OpenAI został włączony." : "Bot OpenAI został wstrzymany.",
          )}
          onTest={(id) => void runAiBotsAction(
            () => api(`/api/trader/ai-bots/${id}/test`, { method: "POST" }),
            "Połączenie bota z OpenAI API działa poprawnie.",
          )}
          onDelete={(id) => void runAiBotsAction(
            () => api(`/api/trader/ai-bots/${id}`, { method: "DELETE" }),
            "Bot OpenAI został usunięty.",
          )}
        />
      ) : null}
