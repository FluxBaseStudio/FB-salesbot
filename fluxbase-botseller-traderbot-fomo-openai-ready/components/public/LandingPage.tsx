...
const copy = {
  pl: {
    ...
    trader: {
      eyebrow: "Drugi system FluxBase",
      title: "FluxBase TraderBot",
      body: "Analiza rynku, paper trading i automatyczne strategie dla rynku memcoinów",
      cta: "Otwórz TraderBota",
      points: ["Rzeczywiste dane rynku", "Limity ryzyka", "Tryb akceptacji admina"],
    },
  ...
  en: {
    ...
    trader: {
      eyebrow: "Second FluxBase system",
      title: "FluxBase TraderBot",
      body: "Market analysis, paper trading and automated strategies for the memecoin market",
      cta: "Open TraderBot",
      points: ["Real market data", "Risk limits", "Admin approval mode"],
    },
    features: (plan: (typeof BOTSELLER_PLANS)[number]) => [
...
export default function LandingPage() {
  ...
  return (
    <main className="marketing-page marketing-v2 lp">
      ...
      <section className="trader-promo-section">
        <div>
          <span className="eyebrow">{t.trader.eyebrow}</span>
          <h2>{t.trader.title}</h2>
          <p>{t.trader.body}</p>
          <Link className="primary-link lp-cta" href="/trader">{t.trader.cta}</Link>
        </div>
        <div className="trader-promo-list" aria-label={t.trader.title}>
          {t.trader.points.map((point) => <span key={point}>{point}</span>)}
        </div>
      </section>
