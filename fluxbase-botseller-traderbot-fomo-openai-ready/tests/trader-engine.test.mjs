...
assert.equal(absolutePercentDifference('0.000000012345', '0.000000012000'), '2.875');
const tinyEntry = calculatePaperEntry({ orderValue: '10', price: '0.000000012345', feePercent: '0', spreadPercent: '0', slippagePercent: '0' });
assert.equal(tinyEntry.effectiveEntryPrice, '0.000000012345');
assert.equal(Number(tinyEntry.quantity) > 0, true);
...
assert.equal(risk({ lockedUntil: '2026-07-11T10:05:00.000Z', now: '2026-07-11T10:00:00.000Z' }).code, 'RISK_COOLDOWN');
assert.equal(risk({ lockedUntil: '2026-07-11T09:55:00.000Z', now: '2026-07-11T10:00:00.000Z' }).allowed, true);
