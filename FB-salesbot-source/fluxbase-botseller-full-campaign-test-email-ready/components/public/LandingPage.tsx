...
const planFeatures = (daily: number, monthly: number, europe: boolean) => [
  `${daily} maili dziennie, ${monthly.toLocaleString("pl-PL")} miesięcznie`,
  europe ? "Cała Europa lub wybrane kraje" : "Cała Polska lub wybrane województwa",
    ...
    <main className="marketing-page marketing-v2 lp">
      ...
      <section className="plans-section plans-section-v2" id="pakiety">
        <div className="section-heading">
          ...
          <p>Stała cena miesięczna, płatność Stripe, aktywacja po ręcznej weryfikacji przez FluxBase.</p>
        </div>
          ...
          {BOTSELLER_PLANS.map((plan) => (
            <article className={plan.recommended ? "plan-card featured" : "plan-card"} key={plan.id}>
              ...
              <div className="lp-plan-price">
                <strong>{plan.pricePln.toLocaleString("pl-PL")} zł</strong>
                <span>/ miesiąc</span>
              </div>
              <ul className="lp-plan-features">
                {planFeatures(plan.dailyEmails, plan.monthlyEmails, plan.scope === "europe").map((feature) => (
