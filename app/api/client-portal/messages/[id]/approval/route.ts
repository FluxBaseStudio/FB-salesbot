...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === "reject") {
      await db.from("messages").update({ status: "failed", last_error: "Odrzucone przez klienta przed wysyłką.", failed_at: new Date().toISOString() }).eq("id", message.id);
    ...
    await db.from("send_queue").insert({
      ...
      lead_payload: {
        ...
        source: lead?.source || "client_approved_draft",
        score: lead?.score || 0,
