...
import { ADDITIONAL_MAILBOX_PRICE_PLN, getPlan, getStripePriceEnvNames, getStripePriceId } from "@/lib/pricing";
...
export async function POST(request: Request) {
  try {
    ...
    const requestedPlan = getPlan(String(payload?.plan_id || ""));
    ...
    if (!termsAccepted || (requestedPlan.billingType === "subscription" && !recurringContractAccepted)) {
      return jsonError(requestedPlan.billingType === "subscription" ? "Przed płatnością wymagane jest zaakceptowanie regulaminu oraz potwierdzenie cyklicznej umowy abonamentowej." : "Przed płatnością wymagane jest zaakceptowanie regulaminu i warunków wersji próbnej.", 400);
    }
    ...
    if (data.additional_mailbox_requested) {
      if (!additionalMailboxPriceId) return jsonError(`Brakuje ceny Stripe dla dodatkowej skrzynki ${ADDITIONAL_MAILBOX_PRICE_PLN} zł netto. Obsługiwane nazwy: STRIPE_PRICE_ADDITIONAL_MAILBOX, STRIPE_PRICE_MAILBOX, STRIPE_PRICE_EXTRA_MAILBOX, STRIPE_ADDITIONAL_MAILBOX_PRICE_ID.`, 500);
      lineItems.push({ price: additionalMailboxPriceId, quantity: 1 });
    }
    const checkoutMode = data.plan.billingType === "one_time" ? "payment" : "subscription";
